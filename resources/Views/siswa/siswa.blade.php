                                    ...
                                    <tbody>
                                        @foreach ($siswass as $Siswas)
                                            <tr>
                                                ...
                                                <td>{{ $Siswas->jenisKelamins->nama }}</td>
                                                <td>{{ $Siswas->alamat }}</td>
                                                <td>
                                                    <a href="{{ route('siswa.show',['siswa'=> $Siswas->id]) }}"
                                                        class="btn btn-sm btn-info">Detail</a>
                                                    <a href=""
                                                        class="btn btn-sm btn-warning">Edit</a>
                                                    <a href="" class="btn btn-sm">
                                                        <form action="{{ route('siswa.destroy', ['siswa' => $Siswas->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('delete')
                                                            <button type="submit"
                                                                class="btn btn-outline-danger btn-sm me-2">DELETE<i
